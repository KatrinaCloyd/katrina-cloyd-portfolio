            ...
            {currentProj &&
                <div>
                    ...
                    <div className={styles.detailImgContainer}>
                        {currentProj.image1 && <img src={currentProj.image1} alt={`screenshot 1 of ${currentProj.title}`} />}
                        {currentProj.image2 && <img src={currentProj.image2} alt={`screenshot 2 of ${currentProj.title}`} />}
                    </div>
                    <p>{currentProj.blurb2}</p>
                    <div className={styles.detailImgContainer}>
                        {currentProj.image3 && <img src={currentProj.image3} alt={`screenshot 3 of ${currentProj.title}`} />}
                        {currentProj.image4 && <img src={currentProj.image4} alt={`screenshot 4 of ${currentProj.title}`} />}
                    </div>
                    <p>{currentProj.blurb3}</p>
                    <button className={styles.backBtn} onClick={() => router.push('/DevWork')}>BACK TO DEV WORK</button>
                </div>}
            {!currentProj && <div> Hmmm.... something went wrong. <br /> Please navigate back to the <Link href='/DevWork'>DEV WORK</Link> page and try again.</div>}
        </>
    )
}
